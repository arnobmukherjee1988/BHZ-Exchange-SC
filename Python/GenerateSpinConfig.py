    ...
    for iy in range (Ly):
      for Skyr_ix in range (NumSkAlongX):
        for Skyr_iy in range (NumSkAlongY):
          ...
          # else:
          #     raise ValueError(SpinConfig_Error)
  
  if (SpinConfig_Type == 'CustomSkyrmion'):
      cx, cy = 2*SkyrDiameter, 2*SkyrDiameter
      area_c = cx*cy

      sx_c = np.zeros(area_c)
      sy_c = np.zeros(area_c)
      sz_c = np.zeros(area_c)
      theta_c = np.zeros(area_c)
      phi_c = np.zeros(area_c)
...
def skyrmion_optization():
    ...
    if beta_critical is not None:
        Sx, Sy, Sz, Theta, Phi = Create_SpinConfig(beta_critical)
        return Sx, Sy, Sz, Theta, Phi, beta_critical
    else:
        # Handle the case when beta_critical is not found
        return None
    
# Function to creat spin spiral state

def generate_spin_spiral():
    # Initialize arrays to store spin components
    Sx = np.zeros((Lx, Ly))
    Sy = np.zeros((Lx, Ly))
    Sz = np.zeros((Lx, Ly))
    ...
    for i in range(Lx):
        for j in range(Ly):
            # Setting theta (polar) and phi (azimuthal) angle theta
            Theta[i, j] = 1.0 ; Phi[i, j] = (qx * i) + (qy * j)           
            # Setting spin components
            Sx[i, j] = cos(Phi[i, j]) * sin(Theta[i, j])
            Sy[i, j] = sin(Phi[i, j]) * sin(Theta[i, j])
            Sz[i, j] = cos(Theta[i, j])

    return Sx, Sy, Sz, Theta, Phi

# import numpy as np

def generate_skyrmion(beta):
    SkyrRadius = int(SkyrDiameter/2.0)
    NumSkAlongX = int(Lx/SkyrDiameter)
    NumSkAlongY = int(Ly/SkyrDiameter)
    TotalSkyrmions = NumSkAlongX * NumSkAlongY
    ...
    sy = np.zeros(area_c)
    sz = np.zeros(area_c)
    theta = np.zeros(area_c)
    phi = np.zeros(area_c)

    sx_l    = np.zeros((Lx, Ly))
    sy_l    = np.zeros((Lx, Ly))
    sz_l    = np.zeros((Lx, Ly))
    theta_l = np.zeros((Lx, Ly))
    phi_l   = np.zeros((Lx, Ly))
    ...
    for iy in range(cy):
        for ix in range(cx):
            i = iy * cx + ix
            r = np.sqrt((ix - ix_o) ** 2 + (iy - iy_o) ** 2)
            
            # Handle division by zero
            if r != 0:
                phi[i] = np.arccos(vorticity * (ix - ix_o) / r) - helicity
                if iy - iy_o < 0.0:
                    phi[i] = -helicity - np.arccos(vorticity * (ix - ix_o) / r)
                if (ix - ix_o) == 0.0 and (iy - iy_o) == 0.0:
                    phi[i] = 0.0 + helicity
                ...
                # Handle division by zero
                if den != 0:
                    theta[i] = 2.0 * np.arctan(num / den) + np.arccos(-polarity)
                    if (ix == 0 or ix == cx - 1) and (iy == 0 or iy == cy - 1):
                        theta[i] = 0.0 + np.arccos(polarity)
        ...
        for i in range(Lx // cx):
            for j2 in range(cy):
                for i1 in range(cx):
                    ...
                    jj = j * cy + j2
                    iii = jj * Lx + ii
                    phi_l[ii,jj] = phi[j2 * cx + i1]
                    theta_l[ii,jj] = theta[j2 * cx + i1]

    for iy in range(Ly):
        for ix in range(Lx):
            i = iy * Lx + ix
            sx_l[ix,iy] = np.sin(theta_l[ix,iy]) * np.cos(phi_l[ix,iy])
            sy_l[ix,iy] = np.sin(theta_l[ix,iy]) * np.sin(phi_l[ix,iy])
            sz_l[ix,iy] = np.cos(theta_l[ix,iy])

    return sx_l, sy_l, sz_l, theta_l, phi_l


# sx, sy, sz, theta, phi = generate_skyrmion()
